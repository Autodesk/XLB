...
def test_indices_masker_jax(dim, velocity_set, grid_shape):
    ...
    test_bc.id = 5
    bc_id, missing_mask = indices_boundary_masker([test_bc], bc_id, missing_mask, start_index=None)

    assert missing_mask.dtype == xlb.Precision.BOOL.jax_dtype

    assert bc_id.dtype == xlb.Precision.UINT8.jax_dtype
    ...
    if dim == 2:
        assert jnp.all(bc_id[0, indices[0], indices[1]] == test_bc.id)
        # assert that the rest of the bc_id is zero
        bc_id = bc_id.at[0, indices[0], indices[1]].set(0)
        assert jnp.all(bc_id == 0)
    if dim == 3:
        assert jnp.all(bc_id[0, indices[0], indices[1], indices[2]] == test_bc.id)
        # assert that the rest of the bc_id is zero
        bc_id = bc_id.at[0, indices[0], indices[1], indices[2]].set(0)
        assert jnp.all(bc_id == 0)
