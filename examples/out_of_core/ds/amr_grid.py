...
class Block:
    ...
    def shape_with_ghost(self):
        return tuple([s // 2 ** self.amr_level for s in self.extent_with_ghost])
    ...
    def initialize_box(
        ...
        for neighbour_block in self.neighbour_blocks:
            ...
            self.neighbour_ghost_boxes[neighbour_block][name] = Box(
                ...
                origin=self.origin,
                spacing=self.spacing,
                amr_level=neighbour_block.amr_level,
                cardinality=cardinality,
                ordering=ordering,
                dtype=dtype,
                device=self.device,
    ...
    #    # Check if blocks side by side by seeing it their union has equal area
    #    new_offset = [min(o1, o2) for o1, o2 in zip(self.offset, block.offset)]
    #    new_extent = [max(o1 + e1, o2 + e2) - new_offset for o1, e1, o2, e2 in zip(self.offset, self.extent, block.offset, block.extent)]
    #    if np.prod(new_extent) != np.prod(self.extent) + np.prod(block.extent):
    #        return False

    #    return True

    @classmethod
    def merge(cls, blocks, amr_level=None):

        # Get new amr level
        if amr_level is None:
            amr_level = blocks[0].amr_level

        ## Check if blocks are mergable
        #if not self.mergable(block):
        #    raise ValueError("Blocks are not mergable.")

        # Get new extent and offset
        new_extent = blocks[0].extent
        new_offset = blocks[0].offset
        for block in blocks[1:]:
            new_offset = np.minimum(new_offset, block.offset)
            new_extent = np.maximum(new_offset + new_extent, block.offset + block.extent) - new_offset
        ...
        new_block = cls(
            ...
            spacing=blocks[0].spacing,
            amr_level=blocks[0].amr_level,
            ghost_cell_thickness=blocks[0].ghost_cell_thickness,
        ...
        for block in blocks:
            for neighbour_block in block.neighbour_blocks:

                # Add neighbour block to new block
                if neighbour_block not in blocks:
                    new_block.add_neighbour_block(neighbour_block)

                # Remove block from neighbour block
                neighbour_block.remove_neighbour_block(block)

                # Add new block to neighbour block
                neighbour_block.add_neighbour_block(new_block)
...
class AMRGrid:
    """An out-of-core Adaptive Mesh Refinement (AMR) grid data structure.
    """
