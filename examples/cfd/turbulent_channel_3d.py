...
def define_boundary_indices(grid, velocity_set):
    # Top and bottom sides of the channel are no-slip; other directions are periodic
    box = grid.bounding_box_indices(remove_edges=True)
    walls = [box["bottom"][i] + box["top"][i] for i in range(velocity_set.d)]
    return walls


walls = define_boundary_indices(grid, velocity_set)
...
def initialize_fields(f_0, grid, velocity_set, precision_policy, compute_backend, u_tau):
    ...
    return initialize_eq(f_0, grid, velocity_set, precision_policy, compute_backend, u=u_init)


f_0 = initialize_fields(f_0, grid, velocity_set, precision_policy, compute_backend, u_tau)
